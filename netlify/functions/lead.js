...
var MemStorage = class {
  ...
  async getAgent(id) {
    const agents2 = await this.getAgents();
    return agents2.find((agent) => agent.id === id);
  }
  async getProperties() {
  ...
  async getFeaturedProperties() {
    ...
    return properties2.filter((property) => property.isFeatured);
  }
  async getProperty(id) {
    const properties2 = await this.getProperties();
    return properties2.find((property) => property.id === id);
  }
  async getProjects() {
    return this.loadJsonData("projects.json");
  }
  async getProject(id) {
    const projects2 = await this.getProjects();
    return projects2.find((project) => project.id === id);
  }
...
var handler = async (event, context) => {
  ...
  try {
    if (event.httpMethod === "POST") {
      const leadData = insertLeadSchema.parse(JSON.parse(event.body || "{}"));
